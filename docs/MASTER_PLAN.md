...
| 16 | **Infra & deploy** (Bicep IaC, GitHub Actions → Linux App Service, run-from-package, post-deploy smoke tests) | ✅ | OIDC bootstrap documented; watch item only. Nightly Table Storage backup shipped 2026-07-14 (`backup:prod` + `nightly-backup.yml` — needs the `AZURE_STORAGE_CONNECTION_STRING` repo secret set once). Staging-slot pre-swap deploys scoped but deferred — B1 doesn't support slots, S1+ is a real cost jump (D6) |
...
| **D6** | Staging-slot pre-swap deploys — every deploy currently cuts over live onto users mid-shift, dropping open Socket.io connections and running post-deploy smoke tests only *after* the bad code is already live | 🔵 **Open — owner decided 2026-07-14 to defer.** Requires an App Service Plan tier upgrade (B1 ~$13/mo → Standard S1+ ~$70+/mo — B1 doesn't support deployment slots at all). Revisit if a bad deploy causes a real production incident, or if the cost becomes easier to justify at higher revenue |
