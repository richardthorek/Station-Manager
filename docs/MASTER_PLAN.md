...
| 15 | **Auth & security** (JWT + brigade tokens + demo bypass, `requireSession` read gate, rate limiting, CSP/Helmet) | 🟡 | Pre-launch review 2026-07-17 ([PRE_LAUNCH_STABILITY_SECURITY_REVIEW_20260717](wiki/developer/history/reviews/PRE_LAUNCH_STABILITY_SECURITY_REVIEW_20260717.md)): anonymous `/api/export` leak **fixed** (F1); brigade-access admin routes hardened + manual rollover authenticated **fixed** (Q29 code fix, F6 — prod `REQUIRE_AUTH=true` still needs owner verification); `/api/ai/*` gated with a dedicated tight rate limiter **fixed** (Q31); remaining Q30 (process-level crash handlers — F3), F7 (socket payload validation) |
