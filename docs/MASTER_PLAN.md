...
- **Q19 — Launch-domain canonical/OG URLs (owner decision).** `frontend/index.html` hard-codes `https://bungrfsstation.azurewebsites.net/` (the *old* app service) in the canonical link and every og:/twitter: URL, and the marketing showcase device frames say `bushietools.au`. Decide the public launch domain, then update canonical/OG/twitter URLs (and re-check `og-image.png` branding) before any public marketing push.
