...
**Last updated:** 2026-07-03 (docs restructure — plan rewritten as forward-intent only)
...
| 5 | **Truck check — manual** (vehicle types, locked standard checklists, zones/equipment, issue lifecycle, member attribution) | ✅ | Cross-brigade comparative reporting (Q4); vehicle mgmt surfacing (Q10) |
| 6 | **Truck check — voice agent (A3)** (hold-to-talk PWA → server-side STT → tool loop → TTS; hardened per F1–F16 review) | 🟡 | iPad on-device verification (Q1); pilot rollout (Q2); VAD/continuous listening (Q14); vision + offline = A4 (Q15) |
| 7 | **Reports & analytics** (dashboard, cross-station, CSV/PDF export) | ✅ | Advanced analytics dashboard #123 (Q13) |
| 8 | **AAR Studio** (AI-facilitated After Action Reviews, cloud sync, collab notes, dedupe/merge) | 🟡 | Print-stylesheet refinements (Q8); CSP shrink blocked on streaming gateway (Q9) |
...
| 13 | **Suite federation — Bushie Tools Phase 1** (SM JWT as suite IdP, `/api/auth/entitlements`, app launcher) | ✅ | Phases 2–3: shared packages, monorepo (Q16, Q17) |
| 14 | **PWA / offline** (service worker, install prompt, offline queue) | ✅ | Double SW-registration log (Q11); deeper offline is part of A4 |
...
| 17 | **Notifications (email/SMS)** #120 | ⬜ | Not started (Q12) |
...
- **Q1 — Voice agent: iPad on-device verification.** The F8 audio fixes (AudioContext resume, autoplay unlock, playback-error surfacing) were built against documented iOS Safari behaviour but have never run on a physical iPad — the primary kiosk device. Run a full voice check on iPad (portrait + landscape), capture the screenshots the PR convention requires, and log any device-specific breakage. *Blocks Q2.*
- **Q2 — Voice agent: pilot rollout.** After Q1, enable the voice check for 1–2 pilot AI-plan brigades and gather feedback (noise handling, phrasing, turn caps). The security hardening (F1–F16) is done; this is now a product-feedback exercise, not an engineering one.
- **Q3 — D1 pricing decisions (owner).** Confirm: trial length (14 vs 30 days, AI included?), AI metering unit (session vs audio-minute for the voice agent), top-up pack size/price, per-org vs per-station billing for district orgs, grant/PO invoicing. *Blocks Q5. Decision, not code.*

### Next

- **Q4 — Truck check: cross-brigade comparative reporting.** The whole point of locked standard checklist items with stable `itemCode`s (TC-D1) is comparing outcomes for the same vehicle type across brigades. Build the aggregation (by `vehicleType` + `itemCode`) and a report view. First genuinely new feature value since the TC rebuild.
- **Q5 — Billing: metered AI overage end-to-end.** `meteredUsageReporter.ts` is a safe no-op until a Stripe meter is configured. After Q3: create the meter, map `UsageRecord` → meter events, verify an overage invoice in test mode. Config + a small amount of glue, not a build.
- **Q6 — C4 remainder: device accounts.** Member invite/activation shipped; formalising `BrigadeAccessToken` into first-class `Device` accounts (named kiosk devices, revocation, per-device audit) is the remaining half of SaaS Phase C.
- **Q7 — Wiki content refresh.** The developer wiki pages were migrated from the old `docs/` tree as-is. Sweep them for staleness (known: deployment-optimization and ci-pipeline pages retain some pre-Linux-migration framing), fix intra-doc links, and add screenshots to the user guide (combine with Q1's iPad session).
- **Q8 — AAR Studio: print-stylesheet refinements.** Last open item of AAR polish (P1). Needs a browser to verify; batch with any AAR visual work.

### Later

- **Q9 — AAR Studio: shrink the `/aar` CSP.** Blocked: live transcription still uses the Azure Speech SDK browser-direct (`wss://*.stt.speech.microsoft.com`), so the CSP must keep provider hosts. Unblocks if AAR moves to a backend speech proxy like the voice agent's (same pattern as `agentSpeech.ts`).
- **Q10 — Truck check: surface vehicle management.** Vehicle CRUD is buried inside the admin dashboard (old TC-6). Small UX task.
- **Q11 — Frontend polish batch.** Known minor UAT leftovers: double service-worker registration log; `/login` and `/profile/:memberId` settle in 3.5–5s vs near-instant elsewhere. Batch these with the next frontend session.
- **Q12 — Notifications (email/SMS) #120.** Event reminders and issue-assignment notifications. Not started; design the provider choice (ACS vs SendGrid vs Twilio) before building. The truck-check issue lifecycle (`assignedTo`) is an obvious first consumer.
- **Q13 — Advanced analytics dashboard #123.** Deeper insights beyond the shipped reports (trends, member engagement scoring, predictive maintenance signals). Value unproven — validate demand with pilot brigades before building.
- **Q14 — Voice agent: continuous listening (VAD).** Replace push-to-talk with voice-activity detection behind the same WS frames (`recognizeSpeech` was designed to be swappable). Optional UX upgrade; gather pilot feedback first.
- **Q15 — A4: vision + offline agent phase.** Camera frames + visual diff against `referencePhotoUrl`; on-device speech for offline sheds. Big phase; do not start until the voice agent has pilot mileage.
...
- **Q17 — T7: suite monorepo consolidation.** Turborepo; Fire Break Calculator as an embedded route; Fire Santa Run as a seasonal peer; converge backends onto Express; one pipeline. Depends on Q16 and a commercial reason to do it. (#558)
- **Q18 — T1 remainder: shared truck-check types.** Fold `Appliance`/`VehicleType`/`ChecklistItem`/`CheckRun`/`CheckResult` into `shared/domain-types.d.ts` the same way the sign-in types were done.
...
| ID | Decision | State |
|---|---|---|
| **D1** | Pricing details: trial length, AI metering unit, top-up sizes, org- vs station-level billing, grant/PO invoicing, AAR-in-Basic?, Fire Break tier placement, Santa Run seasonal billing | 🔵 **Open — owner.** Blocks Q5; see Q3 |
| **D2** | Suite auth standard: SM JWT vs Entra External ID | ✅ Resolved — keep SM JWT (kiosk brigade-token model needs it). Revisit only if cross-origin SSO becomes real (Q16) |
...
| **Bushie Suite** *(planned)* | $29/mo · $290/yr | unlimited | unlimited | AI Pro + all Bushie Tools apps (after Q16/Q17) |
...
| R7 | Low user adoption | Med | High | "Average bushie" ethos, pilot feedback loops (Q2), station champions |
...
| R10 | Rural connectivity | High | Med | PWA offline queue; A4 offline agent phase (Q15) |
