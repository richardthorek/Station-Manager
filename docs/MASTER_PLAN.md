...
|---|---|---|---|
| 1 | **Sign-in & check-in/out** (kiosk, mobile, QR, real-time sync) | ✅ | Access rework: AC-1 personal-link credential, AC-2 visitor sign-ins, AC-3 truck-check share/join + `/truckcheck` gating, AC-5 device accounts all shipped; AC-4 (walk-up sweep) remaining; minor perf polish (Q9) |
...
> **Access & membership model (owner, 2026-07-04).** The walk-up apps (sign-in, truck check) are reachable only with a legitimate credential — a brigade device code, a member's personal link, or the public demo — never a bare URL. *Managing* a brigade (members, links/QR, reports) always needs a signed-in account (free within the Community ≤10-member limit, or paid). Visitors can always sign in but get no history and never count toward `maxMembers`. **Shipped:** `AccessRoute` on `/signin` **and now `/truckcheck`**, **AC-1** personal-link session, **AC-2** visitor sign-ins, **AC-3** truck-check share/join + gating, **AC-5** first-class device accounts (see changelog). The remaining piece (**AC-4**, the walk-up consistency sweep) is folded into Block L1.
...
- **Q34 — Demo mode never sets a station id outside kiosk mode, so `requireSession`-gated reads 401 for anonymous demo visitors (found 2026-07-17 while verifying AC-3).** `StationContext.tsx`'s "normal mode" branch explicitly does `setCurrentStationId(null)` — including when `isDemoActive()` is true — so `api.ts` never sends an `X-Station-Id` header for an anonymous demo visitor, and `requireSession`'s demo bypass (which requires the request to explicitly target `DEMO_STATION_ID`) never matches. Reproduced on `/signin` (member roster silently empty — degrades gracefully to "No members available") and `/truckcheck` (harder failure — "Failed to load vehicles", since appliances aren't optional content there). Only affects the credential-less public demo path; authenticated and kiosk sessions are unaffected. **Fix:** have demo mode explicitly resolve to `DEMO_STATION_ID` (e.g. `getCurrentStationId()` fall back to it when `isDemoActive()`), not rely on the request carrying no station context at all.
