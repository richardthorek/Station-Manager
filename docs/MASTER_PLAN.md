...
- **Q21 — Ops: upload the emergency-facilities dataset to prod.** `npm run facilities:fetch` (2026-07-18: confirmed `services.ga.gov.au` **is** reachable from this sandbox, contrary to the script's own comment — ran it live, 8,888 facilities fetched and the resulting `/api/facilities/lookup` verified end-to-end against real data including a full signup + facility claim). While testing found `fetchEmergencyFacilitiesSnapshot.ts`'s layer→serviceType map had gone stale: GA reordered their `MapServer` layers at some point (layer 1 is now `OTHER_EMERGENCY_MANAGEMENT_FACILITY`, 2 is `POLICING_FACILITY`, 3 is `METRO_FIRE_FACILITY` — the old hardcoded map had these three shifted by one, so every fetch would have mislabeled metro-fire facilities as "other," police as "metro-fire," and "other" facilities as "police"). Fixed by resolving `serviceType` from the layer's **name** (fetched from `${SERVICE_URL}?f=json`) instead of a hardcoded id→type table, and made an unrecognized layer name a hard error instead of a silent mislabel, so a future GA renumbering fails loudly rather than corrupting data again. Still outstanding: `npm run facilities:upload` against **prod** Azure Storage (needs live `AZURE_STORAGE_CONNECTION_STRING`, which this sandbox doesn't have) — an owner/operator action. Until then signup's facility-claim step continues to degrade gracefully to "my unit isn't listed."
