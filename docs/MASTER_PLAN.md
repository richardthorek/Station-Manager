...
- **Q32 — Platform-owner console (SaaS operator view) with tenant-data privacy boundary (owner, 2026-07-17).** Build the operator-facing admin surface for the SaaS owner: one platform-owner account (mine) that can see, across **all** organisations, who the users are, which orgs each user owns/belongs to and in what role, each org's subscription plan + status, and basic **aggregate** usage statistics (e.g. active orgs, members/vehicles/stations per org as counts, AI sessions used vs allowance, sign-in volume, last-active). Extends the existing platform-admin plumbing rather than inventing a parallel one — `PLATFORM_ADMIN_USERNAMES` allowlist + `requirePlatformAdmin` (`middleware/platformAdmin.ts`), the `/api/platform` router (today only claim-conflict review), the `isPlatformAdmin` flag on `/api/auth/me`, and the `/admin/platform` page. 
  **Hard privacy constraint (non-negotiable, the defining requirement):** the platform owner must **never** be able to read a tenant's unique operational data — member PII (names, ranks, member numbers, QR codes), individual sign-in/check-in records, truck-check results, event/AAR content, exports, etc. The console returns **only** account metadata (org name, plan, status, owner/user identities, billing email) and **derived aggregate counts/stats** — the Stripe-dashboard model: the operator sees billing and usage shape, not customer content. Enforce this at the API layer with dedicated read-only aggregate endpoints under `/api/platform` that compute counts/rollups server-side and are structurally incapable of returning row-level tenant records — do **not** reuse or widen the tenant data routes for the platform view. 
  *Design notes:* (a) aggregate stats on very small orgs can be re-identifying (an org of 3 shows near-individual data) — keep the surface to counts and plan/usage shape, avoid per-member breakdowns. (b) Platform-owner access should itself be audited (who viewed what account, when). (c) Consider surfacing this trust boundary to customers ("even we can't see your brigade's data") — it's a selling point to privacy-sensitive brigades. (d) Keep the allowlist the sole grant of platform-owner power; it is not an org role and must never be assignable through normal org/user management.
