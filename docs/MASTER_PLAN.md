...
**Last updated:** 2026-07-03 (Q3 sign-in/landing fixes shipped — queue renumbered; AAR hero hardening now Q1/Q2)
...
|---|---|---|---|
| 1 | **Sign-in & check-in/out** (kiosk, mobile, QR, real-time sync) | ✅ | Minor perf polish (Q12) |
...
| 5 | **Truck check — manual** (vehicle types, locked standard checklists, zones/equipment, issue lifecycle, member attribution) | ✅ | Cross-brigade comparative reporting (Q7); vehicle mgmt surfacing (Q12) |
| 6 | **Truck check — voice agent (A3)** (hold-to-talk PWA → server-side STT → tool loop → TTS; hardened per F1–F16 review) | 🟡 | iPad on-device verification (Q5); pilot rollout (Q6); VAD/continuous listening (Q16); vision + offline = A4 (Q17) |
| 7 | **Reports & analytics** (dashboard, cross-station, CSV/PDF export) | ✅ | Advanced analytics dashboard #123 (Q15) |
| 8 | **AAR Studio — THE HERO** (AI-facilitated After Action Reviews, cloud sync, collab notes, dedupe/merge) | 🟡 | Hero hardening + polish per [AAR review](wiki/developer/history/reviews/AAR_STUDIO_REVIEW_20260703.md) (Q1, Q2, Q4); CSP shrink (Q11) |
...
| 11 | **Stripe billing** (Checkout, Portal, webhooks, trial, audit trail, AI top-up packs) | 🟡 | Metered overage needs a Stripe meter + D1 pricing decisions (Q8); device accounts (Q9) |
| 12 | **AI gateway & metering** (`/api/ai/*`, server-side keys, session allowance) | ✅ | Anthropic adapter is a stub (fine until needed) |
| 13 | **Suite federation — Bushie Tools Phase 1** (SM JWT as suite IdP, `/api/auth/entitlements`, app launcher) | ✅ | Phases 2–3: shared packages, monorepo (Q18, Q19) |
| 14 | **PWA / offline** (service worker, install prompt, offline queue) | ✅ | Double SW-registration log (Q13); deeper offline is part of A4 |
...
| 17 | **Notifications (email/SMS)** #120 | ⬜ | Not started (Q14) |
| 18 | **Documentation** (this restructure: plan + registers + wiki) | 🟡 | User-guide screenshots; content refresh of migrated dev pages (Q10) |
...
- **Q3 — D1 pricing decisions (owner).** Confirm: trial length (14 vs 30 days, AI included?), AI metering unit (session vs audio-minute; see AAR-11 for why per-vend is wrong), top-up pack size/price, per-org vs per-station billing for district orgs, grant/PO invoicing. *Blocks Q8. Decision, not code.*

### Next

- **Q4 — AAR remaining batch: complete the collab promise + lows.** **AAR-21** (room notes appear in no export — contributors never see their words in the record), **AAR-20** (iframe print path; the long-open P1 print-stylesheet item — verify on iPad), and the low-severity sweep (AAR-4, 5, 12, 13, 14, 16, 17, 18, 22, 24), folded into whichever files Q1/Q2 already touch.
- **Q5 — Voice agent: iPad on-device verification.** The F8 audio fixes were built against documented iOS Safari behaviour but have never run on a physical iPad. Run a full voice check on iPad (portrait + landscape), capture the screenshots the PR convention requires, log device-specific breakage. *Blocks Q6; batch with AAR on-device checks from Q1.*
- **Q6 — Voice agent: pilot rollout.** After Q5, enable for 1–2 pilot AI-plan brigades and gather feedback (noise handling, phrasing, turn caps). Hardening (F1–F16) is done; this is product feedback, not engineering.
- **Q7 — Truck check: cross-brigade comparative reporting.** The point of locked standard checklist items with stable `itemCode`s (TC-D1): compare outcomes for the same vehicle type across brigades. Aggregation by `vehicleType` + `itemCode` and a report view.
- **Q8 — Billing: metered AI overage end-to-end.** `meteredUsageReporter.ts` is a safe no-op until a Stripe meter is configured. After Q3: create the meter, map `UsageRecord` → meter events, verify an overage invoice in test mode. Include the AAR-11 session-vs-vend fix.
- **Q9 — C4 remainder: device accounts.** Formalise `BrigadeAccessToken` into first-class `Device` accounts (named kiosk devices, revocation, per-device audit) — the remaining half of SaaS Phase C.
- **Q10 — Wiki content refresh.** Sweep migrated developer pages for staleness (known: deployment-optimization + ci-pipeline pre-Linux framing), and add screenshots to the user guide (combine with Q5's iPad session).

### Later

- **Q11 — AAR Studio: shrink the `/aar` CSP.** Blocked: live transcription still uses the Azure Speech SDK browser-direct (`wss://*.stt.speech.microsoft.com`), so the CSP must keep provider hosts. Unblocks if AAR moves to a backend speech proxy like the voice agent's (`agentSpeech.ts` pattern) — worth revisiting once Q1's token-refresh work is in, since a proxy would solve AAR-6 structurally.
- **Q12 — Truck check: surface vehicle management.** Vehicle CRUD is buried inside the admin dashboard (old TC-6). Small UX task.
- **Q13 — Frontend polish batch.** Known minor UAT leftovers: double service-worker registration log; `/login` and `/profile/:memberId` settle in 3.5–5s vs near-instant elsewhere.
- **Q14 — Notifications (email/SMS) #120.** Event reminders and issue-assignment notifications. Design the provider choice (ACS vs SendGrid vs Twilio) first; the truck-check issue lifecycle (`assignedTo`) is the obvious first consumer.
- **Q15 — Advanced analytics dashboard #123.** Value unproven — validate demand with pilot brigades before building.
- **Q16 — Voice agent: continuous listening (VAD).** Swap push-to-talk for voice-activity detection behind the same WS frames. Gather pilot feedback first.
- **Q17 — A4: vision + offline agent phase.** Camera frames + visual diff against `referencePhotoUrl`; on-device speech for offline sheds. Do not start until the voice agent has pilot mileage.
- **Q18 — T6: suite shared packages.** npm/pnpm workspace, `@rfs/ui` / `@rfs/types` / `@rfs/auth-sdk`; sibling repos consume `/api/auth/entitlements`; SSO redirect for cross-origin apps. (#557)
- **Q19 — T7: suite monorepo consolidation.** Turborepo; Fire Break Calculator embedded; Fire Santa Run seasonal peer; converge backends onto Express; one pipeline. Depends on Q18 and a commercial reason. (#558)
- **Q20 — T1 remainder: shared truck-check types.** Fold `Appliance`/`VehicleType`/`ChecklistItem`/`CheckRun`/`CheckResult` into `shared/domain-types.d.ts` like the sign-in types.
...
| ID | Decision | State |
|---|---|---|
| **D1** | Pricing details: trial length, AI metering unit, top-up sizes, org- vs station-level billing, grant/PO invoicing, AAR-in-Basic?, Fire Break tier placement, Santa Run seasonal billing | 🔵 **Open — owner.** Blocks Q8; see Q3 |
| **D2** | Suite auth standard: SM JWT vs Entra External ID | ✅ Resolved — keep SM JWT (kiosk brigade-token model needs it). Revisit only if cross-origin SSO becomes real (Q18) |
...
| **Bushie Suite** *(planned)* | $29/mo · $290/yr | unlimited | unlimited | AI Pro + all Bushie Tools apps (after Q18/Q19) |
...
| R7 | Low user adoption | Med | High | "Average bushie" ethos, pilot feedback loops (Q6), station champions |
...
| R10 | Rural connectivity | High | Med | PWA offline queue; A4 offline agent phase (Q17) |
