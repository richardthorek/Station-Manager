...
- **Q27 — Harden `/ws/agent-check` ↔ Socket.io upgrade coexistence (latent, found 2026-07-16).** Both `attachAgentCheckWs()` and Socket.io register `'upgrade'` listeners on the same `httpServer`. For a `/ws/agent-check` upgrade, engine.io's default `destroyUpgrade` behaviour schedules a socket kill ~1 s later unless the handshake has already written bytes — but the agent-check handler does several sequential awaits (appliance/station/org/session lookups against Azure Table Storage) *before* calling `wss.handleUpgrade`, so under cold-start/high-latency it can exceed that window and engine.io tears the socket down with a 400. Not the cause of the 2026-07-16 same-origin failure (fixed separately) and not reproduced yet, but a genuine race the tests don't cover (they attach the WS to a bare server with no Socket.io). Options: move the DB validation *after* `handleUpgrade` and close with an error frame instead of an HTTP status (loses the clean 401/403/404 rejections the tests assert), or set engine.io `destroyUpgrade: false` on the Socket.io server (verify it propagates and doesn't leak unhandled upgrades). Low priority; revisit if voice check shows intermittent connect failures after the origin fix.
