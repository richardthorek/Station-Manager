...
2. **TC-1 rebuild:** first-class `VehicleType` (org-scoped, `isStandard` flag, immutable standard items with stable `itemCode`s) + appliance→type link carrying the **agency id**; effective checklist = standard items (locked) + brigade custom items, brigade-reorderable. New twins + factory + routes; register in `api_register.json`. Retire the auto-generic per-appliance template.
3. **TC-3:** attribute runs/results to a roster `memberId` (kiosk free-text fallback).
4. **TC-4:** issue lifecycle (status/assignee) + equipment-officer follow-up view (admin-gated).
