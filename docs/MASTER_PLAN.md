...
| 8 | **AAR Studio — THE HERO** (AI-facilitated After Action Reviews, cloud sync, collab notes, dedupe/merge) | 🟡 | Reliability hardening (AAR-6/8/1/7), report attribution (AAR-19), gateway-appropriate error copy (AAR-9/23), GPS-fallback location (AAR-3) and latched AI-failure banner (AAR-10) shipped 2026-07-03; remaining polish + collab/export batch per [AAR review](wiki/developer/history/reviews/AAR_STUDIO_REVIEW_20260703.md) (Q1, Q3); CSP shrink (Q10) |
...
- **Q1 — AAR hero polish: finish the sell.** AAR-6/8/1/7 (reliability), AAR-19 (report attribution), AAR-9/23 (gateway-appropriate error copy), AAR-3 (quick-start GPS coordinates no longer permanently block the AI's real location name) and AAR-10 (persistent AI failures now latch into a single dismissible banner instead of toasting every 45 s) are shipped — remaining, in rank order: **AAR-15** (sync 409 conflicts detected but invisible while editing), **AAR-11** (metering counts a session per token vend — now more pressing since the AAR-6 refresh timer/reconnects call the vend endpoint more often), **AAR-2** (native `prompt`/`confirm` still used elsewhere — the AAR-1 fix added *better-labelled* confirms, not different dialogs).
