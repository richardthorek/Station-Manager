...
**Document Version:** 3.9 — Stripe billing confirmed live in production; new AI-gateway-not-configured blocker found in AAR Studio re-test
...
- **Server-side AI gateway** (#555) — `backend/src/services/aiGateway.ts`,
  ...
  **Not live in production:** `AZURE_OPENAI_*`/`AZURE_SPEECH_*` aren't set in App
  Service — `POST /api/ai/chat` 503s even on a fully-entitled AI-plan org (UAT
  2026-06-22 PM). Same shape as the Stripe go-live gap below: code is done, this
  is a config/ops task.
...
- **Anonymous data exposure:** `GET /api/reports/*`, `GET /api/truck-checks/appliances`, `GET /api/members` return real data to fully credential-less requests. Needs authentication/station-token gating on read endpoints. User encountered this while testing. **Still open — top priority.**
...
- **AI gateway not configured (new, 2026-06-22 PM):** `POST /api/ai/chat` returns 503 on a fully-entitled AI-plan org — `AZURE_OPENAI_*`/`AZURE_SPEECH_*` env vars are not set in App Service. AAR Studio's transcript-to-findings and report generation don't work on production today. **Promoted to top priority alongside the data-exposure item**, since it now blocks the AI Pro plan's core paid feature even though billing for it works.
- **AAR Studio entitlement gate still missing:** re-confirmed 2026-06-22 PM — no `GET /api/auth/entitlements` call on load; direct-nav still bypasses the plan gate (masked when the org happens to be entitled).
- **Admin UX rough edges:** `/admin/organization` rate-limiter fires too aggressively (~5 loads in 4 min, re-confirmed PM now also on plain page loads); module-toggle shows false "Updated" success; "Add New Vehicle" 403 with no error toast; template 404 hangs dashboard.
