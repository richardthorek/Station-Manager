...
- **Q47 — Enrich seeded vehicle-type templates against proven brigade sheets.** The 2026-07-18 [truck-checklist review](wiki/developer/history/reviews/TRUCK_CHECKLIST_TEMPLATE_REVIEW_20260718.md) found the seeded standard types in `standardVehicleTypes.ts` reproduce only ~40% of a real accepted check sheet and miss safety-critical rows (fire extinguisher, AED, battery security, BA set, air-brake moisture bleed). Work: (R1) add the universally-missing safety rows and procedural bookends and bump `SEED_VERSION`; (R2) populate item `description`s and set `expectedResponseType`/`unit` for gauge readings (tyre `psi`, pump `kpa`, oil `level`); (R3) drop items no sheet uses (`power-steering`, `spare-tyre`, `adblue-level`), collapse the 5 split light rows, add comms/`mdt` + `safety` groups, and add a "Support Tanker / Light Pumper" type for pump-carrying support trucks (LG-class); (R4) doc the per-appliance overlay as the home for per-truck values (pressures/kpa/channels/counts); (R5) add a seed test asserting each RFS type keeps the safety rows. Keep the type-vs-appliance split from finding F5 — universal items in the seed, brigade specifics in the overlay.
